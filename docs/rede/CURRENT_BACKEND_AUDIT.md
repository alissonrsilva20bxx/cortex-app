...
9. **Nenhuma das quatro migrations (`0001`–`0004`) tem `GRANT` explícito para `anon`/`authenticated`** — sempre dependeram do default legado do Supabase de auto-expor toda tabela nova de `public`. Confirmado ao montar o harness de `RD-17`: um Supabase local recriado com o CLI atual (`2.109.1`) nega `permission denied for table jobs` até `auto_expose_new_tables = true` ser habilitado manualmente em `supabase/config.toml` — e esse campo **é removido em 2026-10-30** segundo o próprio aviso do CLI. Não se sabe se o projeto remoto de produção já opera sob o novo default (grants ausentes) ou está "grandfathered" com os grants legados — não verificado, exige auditoria remota de `information_schema.role_table_grants` nos moldes de `RD-000`. Ver `docs/rede/TEST_HARNESS_EVIDENCE.md` §2. **[código + inferência — confirmar no remoto antes de 2026-10-30]**
