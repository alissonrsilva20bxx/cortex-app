...
Fonte primária usada aqui: `lib/database.types.ts` (tipos gerados a partir do schema do Supabase em algum momento no passado — presumivelmente via `supabase gen types`), cruzado com `supabase/migrations/*.sql` e os scripts soltos em `supabase/*.sql`. **Importante (correção desta revisão):** esses tipos gerados são um snapshot de quando foram gerados, não uma leitura do estado atual do banco. Nada nesta seção prova o que existe no Supabase remoto _hoje_ — só o que existia quando o snapshot e os scripts foram escritos. Confirmar o estado atual exige auditoria remota direta, que este agente não tem acesso para fazer.
...
| `receitas_avulsas`   | Idem — só em `supabase/criar_objetivos.sql` (script solto, nome do arquivo não bate com o conteúdo — o arquivo cria _duas_ tabelas: `receitas_avulsas` e `objetivos`). | Sim (no script)                         | Mesma situação: presente nos types gerados, ausente das migrations numeradas. **[código + inferência]**                                                                                                                                                                                                                                                                                                                                |
...
9. **Nenhuma das quatro migrations (`0001`–`0004`) tem `GRANT` explícito para `anon`/`authenticated`** — sempre dependeram do default legado do Supabase de auto-expor toda tabela nova de `public`. Confirmado ao montar o harness de `RD-17`: um Supabase local recriado com o CLI atual (`2.109.1`) nega `permission denied for table jobs` sob o default atual. O harness mantém esse default e usa um grant local estreito em `supabase/seed.sql` apenas para exercitar `jobs`; migrations novas precisam declarar seus próprios grants. Não se sabe se o projeto remoto de produção já opera sob o novo default ou está "grandfathered" com grants legados — não verificado, exige auditoria remota de `information_schema.role_table_grants` nos moldes de `RD-000`. Ver `docs/rede/TEST_HARNESS_EVIDENCE.md` §2. **[código + inferência — confirmar no remoto antes de 2026-10-30]**
