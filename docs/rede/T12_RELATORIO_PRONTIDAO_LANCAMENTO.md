...
> - **`/dev-preview/**`gateado por ambiente**:`middleware.ts`agora bloqueia (404) essas rotas em produção, reaproveitando o mesmo sinal`isDevPreviewEnvironment()`já usado pelo endpoint de bootstrap — inalterado fora de produção (dev local, previews). Teste em`tests/wiring/dev-preview-route-production-gate.test.ts`.
