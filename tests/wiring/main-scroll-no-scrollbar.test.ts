 ...
 * aparelho real: o `<main>` que rola o app (`app/page.tsx`) usava
 * `overflow-y-auto` sem a regra `.no-scrollbar` já existente em
 * `styles/globals.css` (usada por `FilterChips`/`PerfilPublicoScreen`/
 * `JobsTab`), então o Safari/iPhone mostrava a barra ao deslizar.
 *
 ...
 * código-fonte (mesmo padrão dos `*-visual.test.ts`): garante que todo
 * contêiner principal rolável (app real + dev-preview, que precisam ficar
 * coerentes) usa a regra compartilhada -- não uma cópia local que poderia
 * divergir de novo.
 */
...
describe("o <main> rolável do app real usa a regra compartilhada", () => {
