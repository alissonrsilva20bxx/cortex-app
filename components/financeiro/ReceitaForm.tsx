...
export function ReceitaForm({ open, userId, onClose, onSaved }: Props) {
  ...
  async function handleSave() {
    ...
    setLoading(true);
    const { error } = await supabase.from("receitas_avulsas").insert({
      user_id: userId,
  ...
  return (
    ...
    >
      <div
        className="rounded-t-[28px] px-4 pt-5 pb-8"
        style={{
          background: "var(--bg)",
